function zernike_reconstruction(coefficients, radius)
    ...
    % Generate a grid of points within the unit circle
    [x, y] = meshgrid(linspace(-1, 1, 400), linspace(-1, 1, 400));
    [theta, rho] = cart2pol(x, y);
    
    % Mask for points within the unit circle
    mask = rho <= 1;
    ...
    % Scale the surface to the desired radius
    ...
    figure;
    surf(x * radius, y * radius, surface, 'EdgeColor', 'none');
    xlabel('X');
    ylabel('Y');
