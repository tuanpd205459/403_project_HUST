...
% Vẽ hình chữ nhật để chọn ROI
if (chon_vung_HCN == 0)
%    [numRows,numCols] = size(newFourierTransform); 
...
%% Unwrapping phase
thuat_toan_unwrap =1;
switch(thuat_toan_unwrap)
    case 1  % Unwrap LS FD DCT #1
        unWrappedPhase = unwrap_LS_FD_DCT(wrappedPhase);
    case 2  % Unwrap LS FD DCT iter #2
        [unWrappedPhase, num_Iter] = unwrap_LS_FD_DCT_iter(wrappedPhase);
    case 3  % Unwrap LS FD FFT     #3
        unWrappedPhase = unwrap_LS_FD_FFT(wrappedPhase);
    case 4  % Unwrap LS FD FFT iter    #4
        [unWrappedPhase, num_Iter] = unwrap_LS_FD_FFT_iter(wrappedPhase);
    case 5 
        unWrappedPhase = unwrap_TIE_FD_DCT(wrappedPhase);
    case 6      % Unwrap TIE FD DCT iter   #6
        [unWrappedPhase, num_Iter] = unwrap_TIE_FD_DCT_iter(wrappedPhase);
    case 7 
        unWrappedPhase = unwrap_TIE_FD_FFT(wrappedPhase);
    case 8
        [unWrappedPhase, num_Iter] = unwrap_TIE_FD_FFT_iter(wrappedPhase);
    case 9
        unWrappedPhase = unwrap_TIE_FFT_DCT(wrappedPhase);
    case 10         % unwrap TIE FFT DCT iter  #10
         [unWrappedPhase, num_Iter] = unwrap_TIE_FFT_DCT_iter(wrappedPhase);
    case 11         % unwrap TIE FFT FFT   #11
        unWrappedPhase = unwrap_TIE_FFT_FFT(wrappedPhase);
    case 12         % Unwrap TIE FFT FFT iter  #12
         [unWrappedPhase, num_Iter] = unwrap_TIE_FFT_FFT_iter(wrappedPhase);
    otherwise
        disp('Loi chon thuat toan unwrapping')
end


%% Tái tạo bề mặt:
% unwrapped_Phase = unWrappedPhase;                   %unwrap = Possion
% unwrapped_Phase = unwrap_phase(wrappedPhase);      % unwrap = a Linh       
 unwrapped_Phase = phase_unwrap(wrappedPhase);       %unwrap = 2D weight 
...
%% Gọi hàm để hiển thị mặt cắt ngang của cường độ
%figure();
recons = r;
ngang = recons;       
doc = recons'; 
...
figure();
% Kiểm tra xem 'huongMCN' có bằng 'doc' hay không
if isequal(huongMCN, doc)
    chu_huongMCN = 'dọc';    
    imagesc(doc);            
else 
    chu_huongMCN = 'ngang';  
...
title(['Mặt phẳng pha theo chiều (', chu_huongMCN, ')']); % Đặt tiêu đề cho hình ảnh
%colormap(gray);              
